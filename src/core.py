...
def load_and_prepare_data(file_path: Path | None=None, url: str | None=None) -> pd.DataFrame:
...
def fit_forecast_births(series: pd.Series, seasonal_periods: int=4) -> pd.Series:
    ...
    model = ExponentialSmoothing(series, seasonal='add', trend='add', seasonal_periods=seasonal_periods, initialization_method='estimated').fit()
...
def calculate_error_metrics(errors: pd.Series, alpha: float=0.2) -> Dict:
    ...
    return {'mean_error': errors.mean(), 'mad': mad, 'sigma_approx': sigma_approx, 'sample_variance': sample_var}
...
def plot_forecast_analysis(actual: pd.Series, fitted: pd.Series, errors: pd.Series, title: str, output_path: Path, plot: False=False):
    """Plot forecast analysis"""
    if not plot:
        return
    fig, axes = plt.subplots(3, 1, figsize=(10, 12), sharex=True)
    axes[0].plot(actual.index, actual.values, label='Actual', color='#4A90A4', linewidth=1.2)
    axes[0].plot(fitted.index, fitted.values, label='Fitted', color='#D4A574', linewidth=1.2, linestyle='--')
    ...
    axes[2].plot(errors.index, errors.rolling(20).mean(), label='Moving Average (20)', color='#4A90A4', linewidth=1.2)
    axes[2].plot(errors.index, smoothed, label='Exponential Smoothing (α=0.2)', color='#D4A574', linewidth=1.2, linestyle='--')
