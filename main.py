...
def prepare_parser() -> None:
    ...
    fitted_births = fit_forecast_births(
        births["Births"], config["model"]["seasonal_periods"]
    )
    ...
    var_3 = calculate_multi_step_variance(
        config["analysis"]["c1"],
        config["analysis"]["c_tau_3"],
        metrics["mad"],
        config["model"]["alpha"],
    )
    var_6 = calculate_multi_step_variance(
        config["analysis"]["c1"],
        config["analysis"]["c_tau_6"],
        metrics["mad"],
        config["model"]["alpha"],
    )
    var_12 = calculate_multi_step_variance(
        config["analysis"]["c1"],
